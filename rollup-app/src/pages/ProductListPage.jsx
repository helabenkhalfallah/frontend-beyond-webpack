...
const fetchProducts = (page = 1) => {
  return fetch(`https://dummyjson.com/products?skip=${page}&limit=10`)
...
const ProductListPage = () => {
  ...
  const filteredProducts = productDataSource
    ? productDataSource.filter((product) => product.title.toLowerCase().includes(searchTerm.toLowerCase()))
    : [];

  // Calculate the number of items in the list
  const hasNextPage = data ? page < data.total : false;
  const itemsCount =
		(hasNextPage && filteredProducts && filteredProducts?.length ? filteredProducts.length + 1 : filteredProducts?.length) || 0;
